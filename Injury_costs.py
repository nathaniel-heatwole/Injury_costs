...
# Maps MAIS-based costs onto the ISS (ordinal logistic regression, Gaussian naive Bayes) and groups severity ranges using various data features (k-means clustering)
...
iss_breakpoint = 25  # bifurcated iss table

currency_sig_digits = 3

for_publication = 0  # 0 = no / 1 = yes (relates to plot formatting)
...
color_dict = {'cost_Grah_etal_1997':'blue',
              'cost_Fink_etal_2006_qol':'red',
              'cost_DOT_2021':'darkorange',
              'cost_Blin_etal_2023':'black'}

linestyle_dict = {'cost_Grah_etal_1997':'dashdot',
                  'cost_Fink_etal_2006_qol':'dashed',
                  'cost_DOT_2021':'solid',
                  'cost_Blin_etal_2023':'dotted'}
...
def cost_format(df):
    ...
    for var in df_in.columns:
        df_in['oom'] = df_in[var].apply(lambda x: 1 if x == 0 else np.floor(math.log10(abs(x))))  # determine order of magnitude
        df_in['ratio'] = df_in[var] / 10**df_in['oom']                                            # condensed value
        df_in['ratio'] = round(df_in['ratio'], currency_sig_digits - 1)                           # round condensed value
        df_out[var] = df_in['ratio'] * 10**df_in['oom']                                           # expand condensed value back out
        df_out[var] = round(df_out[var], 0)                                                       # remove cents
        df_out[var] = df_out[var].apply(lambda x: x if str(x) == 'nan' else f'${int(x):,.0f}')    # currency format
    return df_out

# dash formatting (missing/nan)
def dash_format(df):
...
# mais shares adjustment (by iss)
def adjust_shares(df):
    df_in = df.copy(deep=True)
    df_out = pd.DataFrame()
    # reset impossible mais-iss combinations to zero
    for m in range(1, 5+1):
        name = 'mais' + str(m) + '_share'
        df_in.loc[df_in['mais_min'] > m, name] = 0
        df_in.loc[df_in['mais_max'] < m, name] = 0
        df_in.loc[df_in['iss'] == 75, name] = 0  # iss 75
    df_in['mais6_share'] = df_in['iss'].apply(lambda x: 1 if x == 75 else 0)  # mais 6 / iss 75
    # renormalize remaining shares to one
    df_in['constant'] = 1 / df_in[mais_1to6_shares].sum(axis=1)
    for m in range(1, 6+1):
        name = 'mais' + str(m) + '_share'
        df_in[name] *= df_in['constant']
    df_out[mais_1to6_shares] = df_in[mais_1to6_shares]
    return df_out
...
training = iss_exploded.loc[iss_exploded['iss'] != 75]  # iss 75 is separate
...
shares_olr = pd.DataFrame(iss[['iss', 'mais_str', 'mais_min', 'mais_max']]).copy(deep=True)
...
# predict shares (by iss)
shares_gnb = iss[['iss', 'mais_str', 'mais_min', 'mais_max']].copy(deep=True)
...
for name in mais_1to6_shares:
    shares[name] = 0.5*(shares_olr[name] + shares_gnb[name])  # olr/gnb average
...
for cost in mais_cost_vars:
    iss_costs[cost] = 0
    for m in range(1, 6+1):  # loop needed because shares are rowwise
        iss_costs[cost] += mais[cost][m-1] * shares['mais' + str(m) + '_share']  # applies shares to mais costs
...
shares['avg_mais'] = 0
for m in range(1, 6+1):  # loop needed because shares are rowwise
    shares['avg_mais'] += m * shares['mais' + str(m) + '_share']  # applies shares to determine average mais
...
table1 = mais[['mais'] + mais_noncost_vars].copy(deep=True)
...
# mais costs
table2 = mais[['mais', 'iss_min', 'iss_max'] + mais_cost_vars].copy(deep=True)
table2[mais_cost_vars] = cost_format(table2[mais_cost_vars])  # formats cost variables

# iss
table3 = pd.DataFrame() 
table3_prelim = iss[['iss', 'dist_copes_etal_1988', 'dist_kilgo_etal_2004', 'pdeath_copes_etal_1988', 'pdeath_kilgo_etal_2004']].copy(deep=True)
...
# mais shares (by iss)
table4 = shares[['iss', 'mais_str', 'avg_mais']].copy(deep=True)
...
# clusters (ranges of mais/iss values)
...
vert_buffer_mais = fraction * 6         # fig2
horz_buffer_mais = fraction * 6         # fig1
horz_buffer_iss = fraction * max_iss    # fig2
vert_buffer_cost = fraction * max_cost  # fig1

fraction_legend = 0.5*horz_buffer_mais / (2*horz_buffer_mais + (6-1))

# colors
legend_color = 'whitesmoke'
plot_area_color = 'white'
if for_publication == 0:
    background_color = 'lightblue'
elif for_publication == 1:
...
# mais-iss feasible region (bounds are rearranged version of mais bounds for iss)
iss_bounds = pd.DataFrame()
iss_bounds['iss'] = np.arange(1, max_iss + 1)          # integers 1-75 (for smoothness)
iss_bounds['mais_min'] = np.sqrt(iss_bounds['iss']/3)  # excludes ceiling function (for the entire curve, not just the integers)
iss_bounds['mais_max'] = np.sqrt(iss_bounds['iss'])    # excludes floor function (for the entire curve, not just the integers)
iss_bounds['mais_min'] = iss_bounds['mais_min'].apply(lambda x: 1 if x < 1 else 5 if x > 5 else x)  # ensures region does not extend below mais 1
iss_bounds['mais_max'] = iss_bounds['mais_max'].apply(lambda x: 1 if x < 1 else 5 if x > 5 else x)  # ensures region does not extend above mais 5
...
plt.ylim(-vert_buffer_cost, max_cost + vert_buffer_cost)
...
plt.legend(loc='upper left', bbox_to_anchor=(fraction_legend, 1 - fraction_legend), fontsize=legend_size, facecolor=legend_color, framealpha=1).set_zorder(20)
plt.grid(True, color='lightgray', linewidth=0.75, alpha=0.5, zorder=0)
plt.show(True)

# feasible region (mais-iss)
fig2, ax = plt.subplots(facecolor=background_color)
plt.gca().set_facecolor(plot_area_color)
if for_publication == 0:
...
plt.legend(loc='lower right', bbox_to_anchor=(1 - fraction_legend, fraction_legend), fontsize=legend_size, facecolor=legend_color, framealpha=1).set_zorder(20)
...
pdf = PdfPages('Injury_costs_graphics.pdf')
for fig in fig_list:
    f = fig_list.index(fig) + 1
    fig.savefig('INJ-figure-' + str(f) + '.jpg', dpi=300)
    pdf.savefig(fig)
pdf.close()
del pdf, fig, f
