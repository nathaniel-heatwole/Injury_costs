...
cost_name_dict = {'cost_Grah_etal_1997':'Graham et al. [29,47]',
...
cost_color_dict = {'cost_Grah_etal_1997':'blue',
                   'cost_Fink_etal_2006_qol':'red',
                   'cost_DOT_2021':'darkorange',
                   'cost_Blin_etal_2023':'black'}

cost_linestyle_dict = {'cost_Grah_etal_1997':'dashdot',
...
iss_bounds['mais_min'] = np.sqrt(iss_bounds['iss']/3)  # excludes ceiling function (to get the entire curve, not just the integers)
iss_bounds['mais_max'] = np.sqrt(iss_bounds['iss'])    # excludes floor function (to get the entire curve, not just the integers)
...
for cost in mais_plot_cost_vars:   
    plt.plot(mais['mais'], mais[cost] / 10**6, label=cost_name_dict[cost], linestyle=cost_linestyle_dict[cost], color=cost_color_dict[cost], linewidth=line_width, zorder=5)  # entire curve
    plt.scatter(mais['mais'], mais[cost] / 10**6, marker='*', color='black', s=6, zorder=10)    # integer points only
...
fig3 = plt.figure()
for cost in mais_plot_cost_vars:
    plt.plot(iss_costs['iss'], iss_costs[cost] / 10**6, label=cost_name_dict[cost], linestyle=cost_linestyle_dict[cost], color=cost_color_dict[cost], linewidth=line_width, zorder=5)
if include_plot_titles == 1:
